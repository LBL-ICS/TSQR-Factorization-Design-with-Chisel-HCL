...
*/
  // 13 cycle adder
  class FP_adder_13ccs(bw: Int, name:Int) extends Module {
    override def desiredName = s"FP_adder_13ccs_${name}"
    require(bw == 16 || bw == 32 || bw == 64 || bw == 128)
    val io = IO(new Bundle() {
    ...
    val subber = Module(new full_subber(exponent,name))
    ...
    // add the fractional/mantissa parts of the ieee numbers together
    val adder = Module(new full_adder(mantissa + 1,name))
    adder.io.in_c := 0.U
    ...
    val subber2 = Module(new full_subber(exponent,name))
  ...
  // 13 cycle subtractor
  class FP_subtractor_13ccs(bw: Int, name:Int) extends Module {
    override def desiredName = s"FP_subtractor_13ccs_${name}"
    require(bw == 16 || bw == 32 || bw == 64 || bw == 128)
    val io = IO(new Bundle() {
      val in_en = Input(Bool())
      val in_a = Input(UInt(bw.W))
    ...
    // the subtraction is just a special case of the adder
    val FP_adder = Module(new FP_adder_13ccs(bw, name))
    val adjusted_in_b = WireInit(0.U(bw.W))
    // the second input needs to have its sign bit inverted for using the adder for subtraction
    adjusted_in_b := (~io.in_b(bw - 1)) ## io.in_b(bw - 2, 0)
  ...
  // 10 cycle multiplier
  class FP_multiplier_10ccs(bw: Int,name: Int) extends Module {
    override def desiredName = s"FP_multiplier_10ccs_${name}"
    require(bw == 16 || bw == 32 || bw == 64 || bw == 128)
    val io = IO(new Bundle() {
      val in_en = Input(Bool())
      val in_a = Input(UInt(bw.W))
    ...
    val multiplier = Module(new multiplier(mantissa + 1,name))
    ...
    // will take twoscomplement of subtraction result
    val complementN = Module(new twoscomplement(exponent,name))
    ...
    // will add the twoscomplement result to the first input exponent
    val adderN = Module(new full_adder(exponent,name))
    adderN.io.in_c := 0.U
  ...
  class FP_square_root_newfpu(bw: Int, NR_iter: Int, name: Int) extends Module {
    ...
    val multipliers = Vector.fill(NR_iter)(Vector.fill(3)(Module(new FP_multiplier_10ccs(bw, name)).io))
    val subtractors = Vector.fill(NR_iter)(Module(new FP_subtractor_13ccs(bw,name)).io)
    ...
    val multiplier4 = Module(new FP_multiplier_10ccs(bw, name))
  ...
  class FP_reciprocal_newfpu(bw: Int, NR_iter: Int, name: Int) extends Module {
    override def desiredName = s"FP_reciprocal_newfpu_${name}"
    require(bw == 16 || bw == 32 || bw == 64 || bw == 128)
    val io = IO(new Bundle() {
      val in_en = Input(Bool())
      val in_a = Input(UInt(bw.W))
    ...
    val multiplier4 = Module(new FP_multiplier_10ccs(bw, name)) // one cycle
    multiplier4.io.in_en := io.in_en
    multiplier4.io.in_a := 0.U(1.W) ## multipliers(NR_iter-1)(2).out_s(bw - 2, 0)
    ...
    val multipliers_r = Vector.fill(NR_iter_r)(Vector.fill(2)(Module(new FP_multiplier_10ccs(bw, name)).io))
    val subtractors_r = Vector.fill(NR_iter_r)(Module(new FP_subtractor_13ccs(bw,name)).io)
  ...
  class FP_divider_newfpu(bw:Int, NR_iter: Int, name: Int) extends Module{
    override def desiredName = s"FP_divider_newfpu_${name}"
    val io = IO(new Bundle{
      val in_en = Input(Bool())
      val in_a = Input(UInt(bw.W))
      val in_b = Input(UInt(bw.W))
    ...
    val reciprocal = Module(new FP_reciprocal_newfpu(bw, NR_iter, name)).io
    reciprocal.in_en := io.in_en
    reciprocal.in_a := io.in_b
    val multiplier = Module(new FP_multiplier_10ccs(bw, name)).io
...
*/
  class FP_DDOT_dp(n: Int, bw: Int, name: Int) extends Module {
    override def desiredName = s"FP_DDOT_dp_${name}"
    val io = IO(new Bundle{
      val in_a = Input(Vec(n, UInt(bw.W)))
      val in_b = Input(Vec(n, UInt(bw.W)))
      val out_s = Output(UInt(bw.W))
    ...
    val multipliers = Vector.fill(n)(Module(new FP_multiplier_10ccs(bw, name)).io)
    //    multipliers.map(x=>x.in_en := io.in_en)
    multipliers.zipWithIndex.map(x=>x._1.in_a := io.in_a(x._2))
    multipliers.zipWithIndex.map(x=>x._1.in_b := io.in_b(x._2))
    ...
    if(add_per_layer.nonEmpty) {
      val regs_and_adds = for (i <- 0 until add_per_layer.length) yield {
        val adds = for (j <- 0 until add_per_layer(i)) yield {
          Module(new FP_adder_13ccs(bw,name)).io
        }
  ...
  class axpy_dp(bw:Int,level:Int, name: Int)extends Module {
    override def desiredName = s"axpy_dp_${name}"
    val io = IO {
      new Bundle() {
        val in_a = Input(UInt(bw.W))
        val in_b = Input(Vec((level), UInt(bw.W)))
    ...
    val multiply_layer = for(i <- 0 until level)yield{
      val multiply = Module(new FP_multiplier_10ccs(bw, name)).io
      multiply.in_en := true.B
      multiply
    }
    val adder_layer = for (i <- 0 until level) yield {
      val adder = Module(new FP_adder_13ccs(bw,name)).io
      adder.in_en := true.B
      adder
    }
    val reg_array_h = Vector.fill(level)(Module(new FPReg(10, 32,name)).io)
    for(i <- 0 until level){
      multiply_layer(i).in_a := io.in_a
      multiply_layer(i).in_b := io.in_b(i)
  ...
  class hqr5(bw: Int, name:Int) extends Module{
    ...
    val adder = Module(new FP_adder_13ccs(bw,name)).io
    val subtractor = Module(new FP_subtractor_13ccs(bw,name)).io
    adder.in_en := true.B
    adder.in_a := io.in_a
    adder.in_b := io.in_b
  ...
  class hqr7(bw: Int, name: Int) extends Module{
    ...
    val multiplier = Module(new FP_multiplier_10ccs(bw, name)).io
    val reciprocal = Module(new FP_reciprocal_newfpu(bw,1, name)).io
 ...
 /* def main(args: Array[String]) : Unit = {
    val sw = new PrintWriter("FP_DDOT_dp.v")
        sw.println(getVerilogString(new FP_DDOT_dp(32, 32, name)))
        sw.close()
    val sw2 = new PrintWriter("axpy.v")
        sw2.println(getVerilogString(new axpy_dp(32, 256, name)))
        sw2.close()
*/
...
//    }
   /* val sw5 = new PrintWriter("hqr7_chisel.v")
    sw5.println(getVerilogString(new hqr7(32, "name")))
    sw5.close()
  } */
