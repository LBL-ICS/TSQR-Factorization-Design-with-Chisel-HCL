...
class fsm(bw:Int, streaming_width:Int, CNT_WIDTH: Int) extends RawModule{
    ...
    val dmx0_mem_wea = IO(Output(UInt((streaming_width*4).W)))
    ...
    val dmx1_mem_wea = IO(Output(UInt((streaming_width*4).W)))
    ...
    val rtri_mem_wea = IO(Output(UInt((streaming_width*4).W)))
    ...
    withClockAndReset(clk,rst){
        val DDOT_CY = 26+13*log2Ceil(streaming_width)//testing
        val HQR3_CY = 139  
        //val HQR3_CY = 29   
        val HQR5_CY = 330 
        //val HQR5_CY = 36
...
/////
        ...
        tk_rdy := RegNext(hh_en & (cnt ===( VK_CY+(DDOT_CY) ).U))
        ...
        d5_rdy := tr_cnt_en & (tr_cnt >= ( DDOT_CY.U + D4_SFT_NO.U - 2.U)) & (tr_cnt < ( DDOT_CY.U + tr_cy + D4_SFT_NO.U -2.U))
        ...
        d4_sft := tr_cnt_en & (tr_cnt >= (DDOT_CY-1).U) & (tr_cnt < ( (DDOT_CY-1+D4_SFT_NO).U+ tr_cy))
        ...
        val dmx0_mem_wea_reg = Reg(UInt((streaming_width*4).W))
        val dmx1_mem_wea_reg = Reg(UInt((streaming_width*4).W))
        val rtri_mem_wea_reg = Reg(UInt((streaming_width*4).W))
        val dmx0_mem_wea_update = Reg(UInt((streaming_width*4).W))
        val dmx1_mem_wea_update = Reg(UInt((streaming_width*4).W))
        val rtri_mem_wea_update = Reg(UInt((streaming_width*4).W))
