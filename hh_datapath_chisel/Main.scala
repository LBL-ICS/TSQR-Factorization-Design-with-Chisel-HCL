...
object Main{
  ...
  def main(args: Array[String]) : Unit = {
    val sw2 = new PrintWriter("hh_datapath_16_chisel.v")
    sw2.println(getVerilogString(new hh_datapath(32,16,16)))
    sw2.close()
  }
}

class hh_datapath(bw:Int, streaming_width:Int, CNT_WIDTH: Int)extends Module {
    val io = IO {
      new Bundle() {
        val clk = Input(Clock())
        val rst = Input(Bool())
    ...
    val yj_reg_1 = Reg(UInt(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8).W))
    val yj_reg_2 = Reg(UInt(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8).W))
    val yj_reg_3 = Reg(UInt(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8).W))
    val yj_reg_4 = Reg(UInt(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8).W))
    ...
    when(io.rst){
      ...
      yj_reg_4 := Cat(yj_reg_3(streaming_width*32-1,0),yj_reg_4)(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8)+streaming_width*32-1,streaming_width*32)
      yj_reg_3 := Cat(yj_reg_2(streaming_width*32-1,0),yj_reg_3)(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8)+streaming_width*32-1,streaming_width*32)
      yj_reg_2 := Cat(yj_reg_1(streaming_width*32-1,0),yj_reg_2)(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8)+streaming_width*32-1,streaming_width*32)
      yj_reg_1 := Cat(io.hh_din,yj_reg_1)(((log2Ceil(streaming_width)*13+10+129+10-2)*streaming_width*8)+streaming_width*32-1,streaming_width*32)
  ...
  val myNewVec = Wire(Vec(streaming_width, UInt(width = bw.W)))
   ...
   for(i <- 0 until streaming_width ){ 
      ddot.in_a(i) := ddot_din_a(streaming_width*32-(i*32)-1,(streaming_width*32-(32*(i+1))))
      ddot.in_b(i) := ddot_din_b(streaming_width*32-(i*32)-1,(streaming_width*32-(32*(i+1))))
    }
   ...
   val myAxpyVec = Wire(Vec(streaming_width, UInt(width = bw.W)))
   for(i <- 0 until streaming_width){
